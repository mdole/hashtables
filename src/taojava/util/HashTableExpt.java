...
public class HashTableExpt {
    ...
    public static void main(String[] args) {
        ...
        // Conver the PrintWriter to a Reporter.
     ...
     * key/value pairs.
     */
    public static void setExpt(PrintWriter pen, 
            HashTable<String, String> dict) {
            ...
            for (int j = 0; j <= i; j++) {
                try {
                    ...
                    if (!str.equals(words[j])) {
                        pen.println("After setting " + words[i] + ", "
                                + words[j] + " no longer yields itself.");
                        dict.dump(pen);
                        return;
                    } // if we didn't get the expected value.
                } catch (Exception e) {
                    pen.println("After setting " + words[i] + ", "
                            + words[j] + " is no longer in the dictionary.");
                    return;
                } // try/catch
            } // for j
        } // for i
