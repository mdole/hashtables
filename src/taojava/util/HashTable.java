...
public class HashTable<K, V> implements Iterable<V> {
     ...
     * in which we get a surpring relationship between values (e.g., all the
     * hash values are 0 mod 32), when expanding the hash table, we incorporate
     * a random number. (Is this likely to make a big difference? Who knows. But
     * it's likely to be fun.)
     * 
     * For experimentation and such, we allow the client to supply a Reporter
     * that is used to report behind-the-scenes work, such as calls to expand
     * the table.
     * 
     ...
     * implemented.
     * 
     * Features to add. [ ] A real implementation of containsKey. [ ] An
     * iterator for the values. [ ] An iterator for the keys.
     */
    ...
    /**
     * Our helpful random number generator, used primarily when expanding the
     * size of the table..
     */
    Random rand;
    ...
    @SuppressWarnings("unchecked")
    int find(K key) {
	int keyIndex = Math.abs(key.hashCode()) % this.pairs.length;
	while (pairs[keyIndex] != null
		&& !((KVPair) pairs[keyIndex]).key.equals(key)) {
	    keyIndex = (keyIndex + (int) PROBE_OFFSET) % this.size;
	}
	return keyIndex;
    } // find(K)
